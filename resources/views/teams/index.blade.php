...
<div style="display:grid; grid-template-columns:repeat(auto-fill,minmax(240px,1fr)); gap:16px;">
    ...
    <a href="{{ route('teams.show',$team) }}" style="text-decoration:none;">
        <div class="card" style="cursor:pointer; transition:all 0.15s;"
            ...
            <h3 style="font-size:15px; font-weight:600; color:#1E1B4B; margin-bottom:4px;">{{ $team->name }}</h3>
            <p style="font-size:12px; color:#9CA3AF;">{{ $team->members->count() }} member{{ $team->members->count()!==1?'s':'' }}</p>
            <div style="margin-top:14px; padding-top:14px; border-top:1px solid #F3F4F6; display:flex; justify-content:space-between; font-size:11px; color:#9CA3AF;">
                <span>{{ $team->tasks->count() }} tasks</span>
